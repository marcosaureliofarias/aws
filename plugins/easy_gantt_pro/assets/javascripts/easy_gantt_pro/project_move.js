...
ysy.pro.projectMove = {
  patch: function () {
    ...
    ysy.data.saver.sendProjects = function () {
      ...
      for (j = 0; j < projects.length; j++) {
        ...
        if (!project.needLoad && !project.issues_count && !project.has_subprojects) continue;
  ...
  extendGanttTask: function (project, gantt_issue) {
    ...
    var canMoveProject = function (project) {
      if (project.has_subprojects) {
        return project.needLoad;
      }
      return !!project.issues_count === project.hadIssue;
    };

...
ysy.main.extender(ysy.view.Widget, ysy.view.BaselinePanel, {
  ...
  tideFunctionality: function () {
    ...
    this.$target.find("#baseline_select").off("change").on("change", function (event) {
      ...
      ysy.settings.baseline._fireChanges(self, "select")
    });
      ...
      ysy.gateway.deleteBaseline(baseline.id, function () {
          ...
          function (response) {
            ...
            if (response.status === 403) {
              errors = ysy.settings.labels.baselines.error_missing_delete_permission;
            } else {
            ...
            dhtmlx.message(ysy.view.getLabel("baselines", "delete_failed") + ": " + errors, "error");
          });
    });
...
ysy.pro.baseline.openCreateModal = function () {
  ...
  var submitFce = function () {
    ...
    ysy.gateway.saveBaseline(name, function (data) {
          ...
          addLocalizedEasyBaselineNotice();
          //var baselines = ysy.data.baselines;
...
EasyGem.extend(ysy.gateway, {
  ...
  saveBaseline: function (name, callback, failCallback) {
    ...
    var urlTemplate = ysy.settings.paths.baselineRoot;
    this.polymorficPost(urlTemplate, null, data, callback, failCallback);
  },
  deleteBaseline: function (baselineID, callback, failCallback) {
    ...
    this.polymorficDelete(urlTemplate, null, callback, failCallback);
  }
});

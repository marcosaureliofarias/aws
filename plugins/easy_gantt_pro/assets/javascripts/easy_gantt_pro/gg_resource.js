    ...
    ysy.pro.ggrm = {
      ...
      loadResources: function (projectId) {
        ...
        if (projectId) {
          ...
          for (var i = 0; i < projects.length; i++) {
            project = projects[i];
            ids.push(project.id);
            if (!start_date || project.start_date && project.start_date.isBefore(start_date)) {
              start_date = project.start_date;
            }
            if (!end_date || project.end_date && project.end_date.isAfter(end_date)) {
              end_date = project.end_date;
            }
          }
        }
      ...
      _loadProjects: function (json) {
        ...
        for (var i = 0; i < json.length; i++) {
          ysy.main._resourcesStringToFloat(json[i].resources_sums);

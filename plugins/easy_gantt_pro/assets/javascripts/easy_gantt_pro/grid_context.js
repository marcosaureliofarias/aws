...
EasyGem.extend(ysy.pro.grid_context, {
  _name: "Grid context",
  patch: function () {
    ...
    gantt.attachEvent("onContextMenu", function (taskId, linkId, e) {
      ...
      // If user is not bulk-editing => deselect all rows, to avoid changing more rows at one time
      if (!ysy.settings.grid_context.bulk) contextClass.unselectAll();
  ...
  checkboxBuilder: function (obj) {
    if (obj.$rendered_type === "project") return '';
  ...
  contextMenuShow: function (event) {
    ...
    var displayContext = function (data, errors, issueIds) {
      ...
      if(out.length === 0) {
        window.showFlashMessage("warning",ysy.settings.labels.warnings.no_permission_edit);
        return;
      }
      ...
      $context_menu.css('left', (render_x + 'px'))
          .css('top', (render_y + 'px'))
          .show();
      ysy.pro.grid_context.setting.ids = ids;
      self.bindEvents($context_menu, out);
    };
  ...
  prepareContextOut: function (data, issueIds) {
    ...
    for (var i = 0; i < properties.length; i++) {
      ...
      if (this.skipContextMenuItem(property, issueIds)) continue;
  ...
  combineAvailable: function (gatheredData, json) {
    var properties = Object.getOwnPropertyNames(this.propertyProcessors);
    var issueData = json.form_attributes.issue;
    const issuesGanttData = gantt.getTask(issueData.id);
    if (issuesGanttData && issuesGanttData.readonly) return {} ;
    for (var i = 0; i < properties.length; i++) {
      var property = properties[i];
      if (!issueData.hasOwnProperty(property) || issueData[property].length === 0) continue;
      var processor = this.propertyProcessors[property];
    ...
    assigned_to: {
      ...
      optionBuilder: function (json) {
        ...
        for (var i = 0; i < json.length; i++) {
          if (json[i]) {
            users = users.concat(json[i].values);
          }
        }
        return users;
      }
  ...
  bindEvents: function ($context, out) {
    $context.find(".gantt-context-leaf:not(.disabled)").on('click', this.changeObjectOnClick);
    this.initAutocomplete($context, out, "assigned_to");
  },

  initAutocomplete: function ($context, out, key) {
    var filtered = out.filter(function (sub) {
      return sub.property === key;
    });
    if (filtered.length === 0) return;
    var folder = filtered[0].folder;
    var searchArray = [];
    for (var index in folder) {
      if (!folder.hasOwnProperty(index)) continue;

      var oneValue = folder[index];
      searchArray.push({
        name: oneValue.name,
        key: key,
    ...
    var $input = $context.find("#easy_gantt_search_" + key);
    $input.ganttcomplete({
      minLength: 0,
      select: function (event, ui) {
        ysy.pro.grid_context.changeObjectOnClick(ui.item);
      },
      source: function (request, response) {
        var matcher = new RegExp($.ui.autocomplete.escapeRegex(request.term), "i");
        response($.map(searchArray, function (val) {
          if (!request.term || matcher.test(val.name)) {
            return val;
          }
        }));
      }
    ...
    $input.on("click", function () {
      $input.ganttcomplete("search", "");
    });
  },

  changeObjectOnClick: function (ui) {
    var $this = $(this);
    var ids = ysy.pro.grid_context.setting.ids;
    var property = $this.closest("ul").data("property") || ui.key;
    var value = $this.data("value") == null ? ui.value : $this.data("value");
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      if (issue.columns) {
        ...
        changeObject.columns[property] = $this.text().length != '' ? $this.text() : ui.name;
      }
  ...
  skipContextMenuItem: function(item, issueIds) {
    if (item === "done_ratio") {
      return this.skipDoneRatio(issueIds);
    }
  },
  skipDoneRatio: function(issueIds) {
    const appSettings = ysy.settings.appSettings;
    const isSkip = appSettings.useStatusForIssueDoneRatio ||
      (!this.allIssuesAreLeaves(issueIds) && appSettings.issueDoneRatioDerived);

    return isSkip;
  },
  allIssuesAreLeaves: function(issueIds) {
    let areLeaves = true;
    for (let i = 0; i < issueIds.length; i++) {
      const isLeaf = !gantt._branches[issueIds[i]];
      if (!isLeaf) {
        areLeaves = false;
        break;
      }
...
EasyGem.schedule.require(function () {
  $.widget("gantt.ganttcomplete", $.ui.autocomplete, {
    _create: function () {
      this._super();
    },
    ...
    _renderMenu: function (ul, items) {
      ...
      $(ul).attr("data-property", "assigned_to");
    },

    _renderItem: function (ul, item) {
      var content = document.createElement("a");
      var contetClass = "gantt-context-leaf";
      if (item.isChecked) contetClass = contetClass + " icon-checked";
      if (item.isDisabled) contetClass = contetClass + " disabled";
      content.setAttribute("class", contetClass);
      content.setAttribute("href", "javascript:void(0);");
      content.innerHTML = ysy.main.escapeText(item.name);
      return $("<li>").html(content).appendTo(ul);
    }
  });
}, "jQueryUI");

...
window.EasyCalendar.manager.registerPlugin("scheduler",function (instance) {
...
dataProcessor.prototype={
	...
	*/
	...
	/**
	* 	@desc: enable/disable mode , when only changed fields and row id send to the server side, instead of all fields
	*     in default mode
	*	@param:   mode - true/false
	*	@type: public
	*/
	...
	*	@param: mode - "row" - based on row selection changed, "cell" - based on cell editing finished, "off" - manual
	*     data sending
	*	@type: public
	*/
	...
	*	@param: verifFunction - function (object) which should verify cell value (if not specified, then value will be
	*     compared to empty string). Two arguments will be passed into it: value and column name
	*	@type: public
	*/
...
scheduler._mouse_coords=function(ev){
	...
	var cellHeight = this.config.hour_size_px;
	...
	pos.y -= getAbsoluteTop(this._obj) + this.xy.nav_height + (this._dy_shift || 0) + this.xy.scale_height - this._els["dhx_cal_data"][0].scrollTop + cellHeight / 4;
	pos.ev = ev;
...
scheduler._on_mouse_up=function(e){
	...
	if (this._drag_mode && this._drag_id){
		...
		var ev=this.getEvent(this._drag_id);
		const issue = this.main.taskData.taskMap.issues[this._drag_event.issue_id] || {};
		// Should set hours to 0 because we want to compare just days without hours
		const allocationDate = new Date(ev.end_date).setHours(0, 0, 0, 0);
		const issueStartDate = issue.start_date ? new Date(issue.start_date).setHours(0,0,0,0) : null;
		const issueEndDate = issue.due_date ? new Date(issue.due_date).setHours(0,0,0,0) : null;
		const inPast = allocationDate < new Date().setHours(0, 0, 0, 0);
		const outOfRange = !!(issueStartDate && allocationDate < issueStartDate) ||
		                   !!(issueEndDate && allocationDate > issueEndDate);
		if (inPast || outOfRange) {
		  let label = outOfRange ? this.main.settings.labels.errorOutOfTaskLimits :
		                           this.main.settings.labels.issueErrors.errorInThePast;
		  if (Object.keys(this._drag_event).length) {
		    scheduler.setEventStartDate(ev.id, this._drag_event.start_date);
		    scheduler.setEventEndDate(ev.id, this._drag_event.end_date);
		    showFlashMessage('error', label, 1500);
		  } else {
		    showFlashMessage('error', label, 1500);
		...
		if (moved && (this._drag_event._dhx_changed || !this._drag_event.start_date || ev.start_date.valueOf()!=this._drag_event.start_date.valueOf() || ev.end_date.valueOf()!=this._drag_event.end_date.valueOf())){
			var is_new = !inPast && this._drag_mode === "new-size";
...
// TODO remove
window.update_view_count=0;
window.render_view_data_count=0;
scheduler.update_view=function(){
  // printout("update_view "+update_view_count++);
	this._reset_ignores();
...
scheduler._render_scales = function(header, data_area){
	...
	var left=0;
	var container = this._els[".easy-calendar-upcoming__calendar-content"];
	header.innerHTML = "";
	for (var i=0; i<count; i++){
		if (!this._ignores[i]){
...
scheduler._reset_scale=function(){
	...
	var data =  this._els["dhx_cal_data"];
	var data_area = this._els["dhx_cal_data"][0];
	var c = this.config;
	var dataAreaMinHeight = 300;
	//container id of ER.com top bar scheduler from easy_scheduler_quick/show teamplate
  const topSchedulerContainerId = 'scheduler_calendar_quick';

  if (data_area.clientHeight > dataAreaMinHeight && this.main.settings.container !== topSchedulerContainerId ) {
    data.forEach(function (elem) {
      EasyGem.readAndRender(function () {
        return elem.lastElementChild.clientHeight;
      }, function (height) {
...
scheduler.init_templates=function(){
	...
	f(scheduler.templates,{
		...
		month_events_link : function(date, count){
			return "<a>View more<span class='tooltip'>"+count+" events</span></a>";
		},
...
scheduler.filter_event = function(id, ev) {
  if (ev._isGlobalEvent) return true;
...
scheduler.render_view_data = function(evs, hold) {
  // printout("render_view_data "+render_view_data_count++,1);
	var full = false;
	if (!evs) {
		full = true;
		if (this._not_render) {
...
scheduler._render_month_link = function(ev){
  if (!scheduler.renderableEvent(ev)) return;
...
scheduler._calc_event_y = function(ev, min_height){
  ...
  scheduler.renderableEvent = function (ev) {
    var availableCalendars = this.options.availableCalendars;

      if (this.main.assigneeData.banned) {
        if (ev.hasOwnProperty("user_id")) {
          if (ev.user_id === this.main.assigneeData.currentId) return false;
        }
        else if (ev.hasOwnProperty("user_ids")) {
          if (ev.user_ids.length === 1) {
            if (ev.user_ids.includes(this.main.assigneeData.currentId)) return false;
          }
        }
      }

    if (this.main.assigneeData.dominant) {
      if (ev.hasOwnProperty("user_id")) {
        if (ev.user_id !== this.main.assigneeData.primaryId) return false;
      }
      else if (ev.hasOwnProperty("user_ids")) {
        if (!ev.user_ids.includes(this.main.assigneeData.primaryId)) return false;
      }
    }

    if (availableCalendars.hasOwnProperty(ev.type)) {
      if (!availableCalendars[ev.type]) return false;
    }
    return true;
  };
scheduler.render_event = function(ev) {
	...
	var width = Math.floor((parent.data.width - menu_offset) / ev_count);
	...
	if (this.config.cascade_event_display) {
		...
		width = Math.floor(parent.data.width - menu_offset - left - right);
	}
    ...
    var icon_types = "icons_" + ((this._edit_id == ev.id) ? "edit" : "select");
		var icons = this.config[icon_types+"_"+ev.type] || this.config[icon_types];
		var icons_str = "";
		...
		for (var i = 0; i < icons.length; i++) {
		  var icon = icons[i];
			...
			icons_str += "<div class='dhx_menu_icon " + icon + this.templates.icon_class(icon)+"' style='" + bg_color + "" + color + "' title='" + this.locale.labels[icon] + "'"+ariaAttr+"></div>";
		}
		var obj = this._render_v_bar(ev, left - menu + 1, top, menu, icons.length * 20 + 26 - 2, "", "", icons_str, true);
...
scheduler._render_v_bar = function (ev, x, y, w, h, style, contentA, contentB, bottom) {
	...
	if (!bottom && scheduler.renderEvent(container, ev, w, h, contentA, contentB)) {
		...
		inner_html += '<div class="dhx_body" style=" width:' + (w - (this._quirks ? 4 : 14)) + 'px; height:' + (h - (this._quirks ? 20 : 30) + 1-5) + 'px;' + bg_color + '' + color + '">' + contentB + '</div>'; // +2 css specific, moved from render_event

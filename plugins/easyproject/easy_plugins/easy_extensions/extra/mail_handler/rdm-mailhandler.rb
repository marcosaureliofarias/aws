...
class RedmineMailHandler
  ...
  attr_accessor :verbose, :issue_attributes, :allow_override, :unknown_user, :default_group, :no_permission_check, :project_from_subaddress,
                :url, :key, :no_check_certificate, :certificate_bundle, :no_account_notice, :no_notification, :skip_ignored_emails_headers_check
  ...
  def initialize
    ...
    optparse = OptionParser.new do |opts|
      ...
      opts.on("--skip-ignored-emails-headers-check", "if present emails will not be headers will not be checked") { |v| self.skip_ignored_emails_headers_check = '1' }

      opts.separator("")
      opts.separator("Task attributes control options:")
...
Examples:
  ...
  def submit(email)
    ...
    data = { 'key'                               => key, 'email' => email,
             ...
             'skip_ignored_emails_headers_check' => skip_ignored_emails_headers_check }

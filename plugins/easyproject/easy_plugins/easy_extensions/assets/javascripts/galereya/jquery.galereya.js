...
(function ($) {
    ...
    function Galereya(options) {
        ...
        var calcParams = function () {
            ...
            gridH = Math.ceil($cells.length / rowCount) * cellH + (rowCount - 1) * self.options.spacing;
            // TODO works only on images with same dimensions
        };
        ...
        var buildDOM = function () {
            ...
            $imgs.wrapAll('<div class="galereya-grid" />').each(function (i, img) {
              ...
              $img.attr('width', self.options.size + 'px');
              $img.attr('height', self.options.size + 'px');
              $img.parent().attr('width', self.options.size + 'px');
              $img.parent().attr('height', self.options.size + 'px');
        ...
        var loadImages = function (index, category) {
            ...
            if (!img.complete) {
                $(img).attr("src", img.src).on("load", onload);
            } else {
                onload.call(img);
            }
            showCell(index);
        ...
        var createSlide = function (visibleIndex) {
            ...
            $img = $('<img class="galereya-slide-img" src="' + data[index].fullsrc + '" alt="' + data[index].title + '" />').on("load", function () {
                $slide.html($img);
        ...
        var addEventListeners = function () {
            $(window).on('resize', Handlers.windowResize);
            $(document).ajaxComplete(Handlers.windowResize);

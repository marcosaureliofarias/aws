			...
			$(document).on('click', settings.feedbackButton, function(){
				...
				$(document).on('click', '#feedback-submit', function() {
					...
					if ($('#feedback-note').val().length > 0) {
						...
						issue		= {};
						issue.description = $('#feedback-note').val() + '<br/><br/><a href="'+post.url+'">'+post.url+'</a>' + '<br/><br/><img src="'+img+'"/><br/><br/>'+ post.browser;
						issue.subject = post.url;
						issue.project_id = 1826;

						var splitImg = img.split(",");
						console.log(splitImg[1]);
						var binaryImg = atob(splitImg[1]);//atob(base64Image);
						...
						var xhr = $.ajax({ //upload image
							url: window.urlPrefix + '/uploads.json',
							contentType: 'application/octet-stream',
							type: 'POST',
							data: ua  //Base64.decode(splitImg[1]),
						});

						xhr.done(function( msg ) {
							issue.uploads = [];
							msg.upload['filename'] = "printscreen.png";
							msg.upload['content_type'] = "image/png";
							console.log(msg.upload);
							issue.uploads.push(msg.upload);

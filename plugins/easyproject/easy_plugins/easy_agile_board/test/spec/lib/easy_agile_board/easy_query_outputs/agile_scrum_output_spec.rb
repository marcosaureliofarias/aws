...
describe EasyAgileBoard::EasyQueryOutputs::AgileScrumOutput do
  ...
  let(:agile_scrum_output) { described_class.new(query) }
  ...
  describe 'kanban output_settings' do
    before(:each) do
      allow(EasySetting).to receive(:value).with('scrum_output_setting', project).and_return(saved_settings)
    end
    ...
    context 'saved as ActionController::Parameters' do
      ...
      it 'return Hash with saved values' do
        expect(agile_scrum_output.kanban_output_settings).to include(scrum_output_setting)
      end
    end

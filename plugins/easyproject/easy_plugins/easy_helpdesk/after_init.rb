EasyExtensions::PatchManager.register_easy_page_helper 'EasyHelpdeskProjectsHelper'
EasyExtensions::PatchManager.register_easy_page_controller 'EasyHelpdeskController'

EpmEasyHelpdeskMailboxes.register_to_page('easy-helpdesk-overview', :plugin => :easy_helpdesk)
EpmEasyHelpdeskMailtemplates.register_to_page('easy-helpdesk-overview', :plugin => :easy_helpdesk)
EpmEasyHelpdeskProjectQuery.register_to_page('easy-helpdesk-overview', :plugin => :easy_helpdesk)
EpmEasyHelpdeskCreateMailboxButton.register_to_page('easy-helpdesk-overview', :plugin => :easy_helpdesk)
EpmEasyHelpdeskCreateMailTemplateButton.register_to_page('easy-helpdesk-overview', :plugin => :easy_helpdesk)

EasyExtensions::AfterInstallScripts.add do
  page = EasyPage.where(:page_name => 'easy-helpdesk-overview').first
  page_template = page.default_template

  unless page_template
    page_template = EasyPageTemplate.create(:easy_pages_id => page.id, :template_name => 'Default template', :description => 'Default template', :is_default => true)

    EasyPageTemplateModule.create_template_module(page, page_template, EpmNoticeboard.first, 'top-left', HashWithIndifferentAccess.new(:text => '<h1><a class="easy-demo-tutor-youtube-link" href="https://www.youtube.com/watch?v=Y74oLYta4hQ"><img alt="" src="data:image/png;base64," style="border:0px solid black; float:left; height:79px; margin-bottom:0px; margin-left:20px; margin-right:20px; margin-top:0px; width:114px" /></a>Help Desk Dashboard</h1>
<p>Welcome to ' + EasyExtensions::EasyProjectSettings.app_name + ' Help Desk. This is Help Desk Dashboard, where users (both support staff and&nbsp;managers) find <strong>opened help desk tickets grouped by status - <span style="color:#FF8C00">these are tickets that should be processed</span>.</strong>&nbsp;</p>'), 1)
  end

  EasyPageZoneModule.create_from_page_template(page_template) if !page.all_modules.exists?
end


ActiveSupport.on_load(:easyproject, yield: true) do

  require 'easy_helpdesk/hooks'
  require 'easy_helpdesk/internals'
  require 'easy_helpdesk/proposer'

  require 'easy_helpdesk/permissions'

  # To prevent circular dependency error
  require_dependency 'easy_helpdesk_mail_handler'

  EasyHelpdesk.ensure_all_features

  require 'easy_helpdesk/menus'
end

RedmineExtensions::Reloader.to_prepare do

  EasySetting.map.boolean_keys(*EasyHelpdesk.easy_setting_booleans)
  EasySetting.map.boolean_keys(:easy_helpdesk_ignorate_suspend_statuses)

  EasyQuery.map do |query|
    query.register 'EasyHelpdeskProjectQuery'
    query.register 'EasySlaEventQuery'
  end
end

class ReUseCase < ActiveRecord::Base
  ...
  has_one :primary_actor_relation, -> { where(re_artifact_relationships: { relation_type: ReArtifactRelationship::SYSTEM_RELATION_TYPES[:pac] }) },
  ...
  has_many :actor_relations, -> { where(re_artifact_relationships: { relation_type: ReArtifactRelationship::SYSTEM_RELATION_TYPES[:ac] }) },
    :source => :relationships_as_source,
    :through => :re_artifact_properties,
    :class_name => "ReArtifactRelationship"

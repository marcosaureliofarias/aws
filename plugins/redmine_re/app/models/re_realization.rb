class ReRealization < ActiveRecord::Base
  ...
  def self.open_artifacts(project)
    ...
    artifacts_with_issue = ReArtifactProperties.where('id in (select distinct re_artifact_properties_id from re_realizations) AND project_ID = ?', project.id)
  ...
  def self.artifacts_without_issues
     ReArtifactProperties.where('id not in (select distinct re_artifact_properties_id from re_realizations)')
  end

  def self.artifacts_without_issues_by_project(project)
    ReArtifactProperties.where('id not in (select distinct re_artifact_properties_id from re_realizations) AND project_id = ?', project.id)
  end

  def self.issues_without_artifacts
    Issue.where('id not in (select distinct issue_id from re_realizations)')
  end


  def self.issues_without_artifacts_by_project(project)
    Issue.where('id not in (select distinct issue_id from re_realizations) AND project_id = ?', project.id)
  end

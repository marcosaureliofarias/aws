...
class RedmineReController < ApplicationController
  ...
  before_action :first_load, except: [:new, :create]
  before_action :load_settings, :authorize
  # prepend_before_action :first_load, :except => [:configure, :export]
  prepend_before_action :find_project

  def first_load
    @project_artifact = ReArtifactProperties.find_by(artifact_type: "Project", project_id: @project.id)
    if @project_artifact.nil?
      redirect_to controller: "re_settings", action: "new", project_id: @project.id
    else
  ...
  def initialize_tree_data
    ...
    @project_artifact = ReArtifactProperties.find_by(project_id: @project.id, artifact_type: "Project")
  ...
  def find_project
    ...
    if (params[:project_id])
      ...
      artifact = ReArtifactProperties.find_by(id: params[:id])
      if artifact.nil?
  ...
  def create
    ...
    create_hook params
      ...
      @artifact.attributes = re_artifact_params
      ...
      if valid && @artifact_properties.valid?
        ...
        unless @sibling.nil?
          @artifact_properties.parent_relation.position = @sibling.parent_relation.position + 1
        end
  ...
  def find_first_artifacts_with_first_parameter(filter_param, artifact_type)
    ...
    if artifact_properties_attribute # ReArtifactProperties.has_attribute?(filter_param[0])
      artifacts += ReArtifactProperties.like(filter_param[0] => filter_param[1] + '%').like(:artifact_type => artifact_type)
                                     # attribute is a special one used by one of the subclasses of ReArtifactProperties
    else
      cond = { filter_param[0] => filter_param[1] }
      case artifact_type
        when "ReSubtask", ""
          artifacts += ReSubtask.where(cond)
        when "ReTask", ""
          artifacts += ReTask.where(cond)
        when "ReGoal", ""
          artifacts += ReSubtask.where(cond)
      end
    end
  ...
  def create_tree(re_artifact_properties)
    ...
    artifact_name = "(#{re_artifact_properties.id}) #{artifact_name}" if ReSetting.display_requirement_id?(@project)
    if re_artifact_properties.re_status.present?
      artifact_name += " <span style='color: #{re_artifact_properties.re_status.color}'>#{re_artifact_properties.re_status}</span>".html_safe
    end
    ...
    unless artifact_type.eql?('project')
      a_attr['data-href'] = re_artifact_property_path(artifact_id)
      a_attr['data-delete-href'] = re_artifact_property_path(artifact_id)
    end
    node['a_attr'] = a_attr

class ReArtifactRelationshipController < RedmineReController
  ...
  def autocomplete_sink
    ...
    @sinks = ReArtifactProperties.where('lower(name) LIKE ? AND project_id = ? AND artifact_type = ?', query.downcase, @project.id, 'Project')
    if @artifact
  ...
  def build_json_for_visualization(artifacts, relations)
    ...
    case @visualization_type
      when "sunburst"
        ...
        re_artifact_properties = ReArtifactProperties.find_by(project_id: @project.id, id: session[:visualization_artifakt_id])
      ...
      when "netmap"
        min_dis_artifact(session[:visualization_artifakt_id].to_i)
        find_all_artifacts_for_netmap(ReArtifactProperties.find_by(project_id: @project.id, id: session[:visualization_artifakt_id]))
    ...
    if (@max_deep.to_i == 0 || @current_deep.to_i <= @max_deep.to_i)
      if(! @issues.include? issue_id)
        ...
        ReRealization.where("issue_id = ?", issue_id.to_s).each do |new_artifact|
          next unless (@chosen_artifacts.include? ReArtifactProperties.find(new_artifact.re_artifact_properties_id).artifact_type.to_s)
          if((! artifacts.include? new_artifact.re_artifact_properties_id.to_s) && (! @found_artifakts.include? new_artifact.re_artifact_properties_id))
            find_all_artifacts_for_netmap(ReArtifactProperties.find_by(project_id: @project.id, id: new_artifact.re_artifact_properties_id.to_s))
          end
        end
      end
    end
  ...
  def graph(artifact_id,first_run)
    ...
    re_artifact_properties = ReArtifactProperties.find_by(project_id: @project.id, id: artifact_id)
    ...
    if (@max_deep.to_i == 0 || @current_deep.to_i < @max_deep.to_i)
      ...
      ReRealization.where("issue_id=?", issue_id.to_s).each do |artifact|
        if( ! @done_artifakts_id.include? artifact.re_artifact_properties_id.to_s)
          ...
          child = ReArtifactProperties.find_by(project_id: @project.id, id: artifact.re_artifact_properties_id)
    ...
    if (@max_deep.to_i == 0 || @current_deep.to_i <= @max_deep.to_i)
      if ( ! @done_artifakts_id.include? artifact.id.to_s)
          ReArtifactRelationship.where(source_id: artifact.id).each do |source|
            next unless (@chosen_artifacts.include? ReArtifactProperties.find(source.sink_id).artifact_type.to_s)
            next unless (@chosen_relations.include? ReArtifactRelationship.find(source.id).relation_type.to_s)
            ...
            relation_settings = ReRelationtype.find_by(project_id: @project.id, relation_type: source.relation_type)
          ...
          ReArtifactRelationship.where(sink_id: artifact.id).each do |source|
            next unless (@chosen_artifacts.include? ReArtifactProperties.find(source.source_id).artifact_type.to_s)
            next unless (@chosen_relations.include? ReArtifactRelationship.find(source.id).relation_type.to_s)
            if (@current_deep.to_i  == @max_deep.to_i)
        ...
        master_build["name"] = ReArtifactProperties.find(artifact.id).name.to_s

class ReRatingsController < ApplicationController
  ...
  def create
    @rap = ReArtifactProperties.find(params[:re_artifact_properties_id])
  ...
  def update
    @rap = ReArtifactProperties.find(params[:re_artifact_properties_id])
    @rating = User.current.re_ratings.find_by(re_artifact_properties_id: @rap.id)
    if @rating.update(params[:re_rating])
      respond_to do |format|

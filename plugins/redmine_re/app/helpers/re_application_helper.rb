module ReApplicationHelper
  ...
  def get_relationtype_html(f)
    new_object = ReRelationtype.new(:is_system_relation => 0, :is_directed => 1, :in_use => 1, :color => "#000000")
    f.fields_for("re_relationtypes", new_object, :index => "new_re_relationtype") do |builder|
  ...
  def get_escaped_relationtype_html(f)
    escape_javascript get_relationtype_html(f)
  end

  # renders a link to javascript to add an empty object into a forms
  def get_escaped_status_html(f)
    new_object = ReStatus.new(:in_use => 1, :color => "#000000")
    fields = f.fields_for("re_statuses", new_object, :index => "new_re_status") do |builder|
      render("re_settings/add_status", :f => builder)
    end
    escape_javascript(fields)
  end
end
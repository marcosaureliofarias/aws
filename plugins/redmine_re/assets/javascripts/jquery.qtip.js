...
function QTip(target, options, id, attr)
{
	...
	function assignEvents()
	{
		...
		*/
			...
			targets.hide.on('mouseleave'+namespace, function(event) {
				clearTimeout(self.timers.show);
			});
		}
		...
		$.each(events.hide, function(index, type) {
			...
			else { targets.hide.on(type+namespace, hideMethod); }
		});
	...
	function unassignEvents()
	{
		...
		else { options.show.target.off(namespace+'-create'); }
	}
...
function Ajax(api)
{
	...
	$.extend(self, {
		init: function() {
			...
			if(opts && opts.url) {
				tooltip.off(namespace)[ opts.once ? 'one' : 'bind' ]('tooltipshow'+namespace, self.load);
			}
	...
	$.extend(self, {
		...
		create: function()
		{
			...
			if(hasCanvas) {
				...
				$('*', elems.tip).on('click mousedown', function(event) { event.stopPropagation(); });
			}
		},
		...
		destroy: function()
		{
			...
			if(delBlanket) {
				...
				if(delBlanket) {
					...
					$(document).off(globalNamespace);
				}
				else {
					elems.overlay.off(globalNamespace+api.id);
				}

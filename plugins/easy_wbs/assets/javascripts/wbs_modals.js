...
(function () {
  ...
  WbsModals.prototype.onEditRequested = function (nodeId) {
    ...
    const rootIdea = this.ysy.idea;
    const save = rootIdea && rootIdea.canUndo();
    if (idea.attr.entityType === "project") {
      if(window.EasyVue && EasyVue.showModal){
        if (!save) {
          EasyVue.showModal(idea.attr.entityType, preFill.id);
        }
        else {
          this.saveAndOpen(idea.attr.entityType, preFill.id);
        }
        return true;
      }
    } else {
  ...
  WbsModals.prototype.saveAndOpen = function (entity, id) {
    var ysy = this.ysy;
    ysy.saver.save("pre-modal-save");
    const showModal = function () {
      EasyVue.showModal(entity, id);
    };
    const unregistEvent = function () {
      ysy.eventBus.unregister("IdeaConstructed", showModal);
    };
    document.addEventListener("vueModalIssueOpened", unregistEvent);
    document.addEventListener("vueModalProjectOpened", unregistEvent);
    ysy.eventBus.register("saveError", unregistEvent);
    ysy.eventBus.register("IdeaConstructed", showModal);
  };
  ...
  WbsModals.prototype.openNewIssueModal = function (preFill) {
    ...
    var successCallback = function (data) {
      ...
      $form.find('#attachments-form').remove();
  ...
  WbsModals.prototype.finishModal = function (actionType, entityType, $form, preFill) {
    ...
    if (ERUI.isMobile) {
      ...
      recalculateModalHeight($target, $buttonpane);
  ...
  WbsModals.prototype.transformData = function (data) {
    ...
    for (var i = 0; i < issueKeys.length; i++) {
      ...
      if (functionMap.hasOwnProperty(key)) {
        parsed = functionMap[key](structured.issue[key], key);
      } else {

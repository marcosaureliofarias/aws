...
(function () {
  ...
  WbsStyles.prototype.styleSources = {
    ...
    spentTimeRatio: {
      init: function () {
        ...
        this.colors = colors;
        this.data = [0, 20, 40, 60, 80, 100, 120, 140, 160, 180];
      },
      dataArray: "spentTimeRatio",
      value: function (data) {
        if (data.isProject) return "project";
        var maxTimeRatio = 180;
        var minTimeRatio = 0;
        var spentTimeRatio;
        if (!data.spent_hours || !data.estimated_hours) return minTimeRatio;

        spentTimeRatio = Math.round(((data.spent_hours / data.estimated_hours) / 20) * 100) * 20;
        if (spentTimeRatio > maxTimeRatio) return maxTimeRatio;

        return spentTimeRatio;
      },
      builderType: "spentTimeRatio"
    },
    milestone: {
      dataArray: "versions",
      value: function (data) {
    ...
    budget: {
      init: function () {
        this.colors = {'neutral': 3, 'negative': 1, 'positive': 2};
        var legend = this.ysy.settings.labels.legend;
        this._options = [
          {name: legend.negative, id: "negative"},
          {name: legend.neutral, id: "neutral"},
          {name: legend.positive, id: "positive"}
        ];
      },
      value: function (data) {
        ...
        if (data.hasOwnProperty('budgets')) {
          value = data.budgets[ysy.settings.budgetType];
        }
        if (budgetType) {
        ...
        return sumIsPositive ? 'positive' : sumIsNegative ? 'negative' : 'neutral';
      },
      options: function () {
        return this._options;
      },
      builderType: "dataBased"
    }
  };
  ...
  WbsStyles.prototype.cssClasses = function (node) {
    ...
    if (node.attr && node.attr.entityType === "project") {
      if (this.ysy.settings.moneyOn) {
        return "" + this.styles["budget"].addSchemeClassFromData(data);
      }
      return " mindmup-scheme-project";
    }
    return ""
        ...
        + this.styles["spentTimeRatio"].addSchemeClassFromData(data)
  };

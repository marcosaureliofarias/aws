...
(function(factory) {
    ...
    $.fn.orgchart = function(options) {
        ...
        if (opts.draggable) {
            $chart.on('dragstart', '.node', function(event) {
                ...
                origEvent.dataTransfer.setData('text', $dragged.attr('id'));

                $chart.trigger({ 'type': 'nodedragstart.orgchart', 'draggedNode': $dragged, 'childrenState': getNodeState($dragged, 'children') });
            })
                .on('dragover', '.node', function(event) {
                    event.preventDefault();
                    if (!$(this).is('.allowedDrop')) {
                        event.originalEvent.dataTransfer.dropEffect = 'none';
                    }
                })
                .on('dragend', '.node', function(event) {
                    $chart.find('.allowedDrop').removeClass('allowedDrop');
                    $chart.trigger({ 'type': 'nodedragend.orgchart', 'draggedNode': this });
                })
                .on('drop', '.node', function(event) {
                    event.preventDefault();
                    event.stopPropagation();

                    var $dropZone = $(this);
                    var $dragged = $chart.data('dragged');
                    $chart.find('.allowedDrop').removeClass('allowedDrop');
                    ...
                    $chart.trigger({ 'type': 'nodedropped.orgchart', 'draggedNode': $dragged, 'dragZone': $dragZone.children(), 'dropZone': $dropZone });
                });
        }


        $chart.on('mouseenter', '.node', function(){
            var $node = $(this),
                nodeState = getNodeState($node, 'children'),
                $bottomEdge = $node.children('.bottomEdge');

            if ($bottomEdge.length) {
                $bottomEdge.addClass('icon').toggleClass('icon-add', !nodeState.visible).toggleClass('icon-remove', nodeState.visible);
            }
        });

        $chart.on('mouseleave', '.node', function(){
            $(this).children('.edge').removeClass('icon icon-remove icon-add');
        });

        // bind click event handler for the bottom edge
        $chart.on('click', '.bottomEdge', function(event) {
            event.stopPropagation();
            var $that = $(this);
            ...
            if (childrenState.exist) {
                ...
                $that.toggleClass('icon-add', childrenState.visible).toggleClass('icon-remove', !childrenState.visible);
            }
        });
    ...
    function createNode(nodeData, level, opts) {
        ...
        if(level === 0) {
            $nodeDiv.addClass('node-root');
        }

        if(nodeData.children) {
            $nodeDiv.addClass('node-parent');
        ...
        if (opts.verticalDepth && (level + 2) > opts.verticalDepth) {
            ...
            if(Number(flags.substr(2,1))) {
                $nodeDiv.append('<i class="edge verticalEdge bottomEdge"></i>');
            }
        }
        ...
        if (opts.createNode) {
            opts.createNode($nodeDiv, nodeData, opts);
        }
        dtd.resolve($nodeDiv);
        return dtd.promise();
    }
        ...
        buildChildNode.call($node.closest('.orgchart').parent(), $node.closest('table'), data, opts, function() {
            if (++count === data.children.length) {
                if (!$node.children('.bottomEdge').length) {
                    $node.append('<i class="edge verticalEdge bottomEdge"></i>');
                }
        ...
        if ($nodeChart.parent().is('td')) {
            ...
            buildChildNode.call($nodeChart.closest('.orgchart').parent(),$nodeChart.parent().closest('table'), nodeData, opts, function() {
                if (++childCount === newSiblingCount) {
                    var $parentTable = $nodeChart.parent().closest('table'),
                        $parentTableBody = $parentTable.children('tbody').length == 0 ? $parentTable : $parentTable.children('tbody'),
                        $siblingTds = $parentTableBody.children('tr:last').children('td');

class EasyGanttEasyIssueQuery < EasyIssueQuery
  ...
  def self.tagged_queries(user = nil, project = nil, options = {})
    queries = super
    queries.each{|q| q.opened_project = project }
    queries
  end
  ...
  def initialize_available_columns
    ...
    @available_columns.keep_if { |column|
      ...
      (column.name.to_s.start_with?('projects.cf_') && !column.custom_field.format_in?(*EasyGantt.cf_formats_to_delete_from_query))
    }
  end
  ...
  def project_scope
    ...
    if opened_project
      ...
      scope = scope.or(old_scope.where(id: opened_project.children.select(:id))) if Setting.display_subprojects_issues?
    end

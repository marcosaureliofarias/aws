...
ysy.view.initGantt = function () {
  ...
  EasyGem.extend(gantt.config, {
    ...
    //date_grid: "%j %M %Y",
    date_format: ysy.main.toMomentFormat(ysy.settings.dateFormat),
    date_grid: "%j.%n.%Y",
    links: {
...
ysy.view.applyGanttPatch = function () {
  ...
  $.extend(gantt.templates, {
    ...
    task_class: function (start, end, task) {
      ...
      const hasStartDate =  model.hasOwnProperty("start_date") && model.start_date;
      const hasDueDate =  model.hasOwnProperty("end_date") && model.end_date;
      const hasStartDateCss = hasStartDate ? "" : "no-start-date";
      const hasDueDateCss = hasDueDate ? "" : "no-due-date";
      css += ` ${hasStartDateCss} ${hasDueDateCss}`;
    ...
    drag_link: function (from, from_start, to, to_start) {
      ...
      } else if (to && gantt.getTask(to).$rendered_type === "project") {
        reason = "project_link"
      } else {
        reason = "other";
      }
  ...
  function initTodayMarker() {
    const userTimeZoneOff = EASY.currentUser.time_zone;
    const date_to_str = gantt.date.date_to_str(gantt.config.task_date);
    const start_date = new Date((new Date().getTime() + (new Date().getTimezoneOffset() * 60000)) + (60000 * userTimeZoneOff))
    const id = gantt.addMarker({start_date, css: "today", title: date_to_str(start_date)});
...
//##################################################################################
  ...
  gantt.attachEvent("onBeforeLinkAdd", function (id, link) {
    ...
    for (var i = 0; i < relArray.length; i++) {
      ...
      if (relation.source_id === sourceId && relation.target_id === targetId) {
        dhtmlx.message(ysy.view.getLabel("errors2", "duplicate_link"), "error");
        return false;
      }
    }
    var stack;
    var source = gantt.getTask(sourceId);
    if (stack = gantt.checkLoopedLink(source, "all", [], targetId)) {
      stack.push(source);
      stack.reverse();
      dhtmlx.message(ysy.view.getLabel("errors2", "loop_link") + " (" + stack.map(function (issue) {
        return issue.text;
      }).join(" -> ") + ")", "error");
      return false;
    }
    var target = gantt.getTask(targetId);
    if (!gantt.isSpaceForLink(link.type, source, target)) {
      dhtmlx.message("Cannot create link, milestone would be crossed", "error");
      return false;
    }
    return true;
  });
  gantt.attachEvent("onAfterLinkAdd", function (id, link) {
    ...
    ysy.pro.relations.moveByCreatedLink(link, rel, true);
  });
  ...
  dhtmlx.dragScroll = function () {
    ...
    if (!$background.hasClass("inited")) {
      ...
      dnd.attachEvent("onDragStart", function () {
        temp = {};
        ysy.proManager.forLastPro(function (callback) {
          return callback(temp, dnd);
        }, "onDragStart");
      });
  ...
  ysy.proManager.register("onDragMove", function (temp, dnd) {
    gantt.scrollTo(Math.max(temp.lastScroll.x - dnd.getDiff().x, 0), undefined);
    return true;
  });
  ...
  ysy.view.bars.registerRenderer("task", function (task, next) {
    ...
    if (this.hasChild(task.id) || $div.hasClass("parent")) {
      ...
      var zoomDays = 1;
      if (gantt._tasks.unit === "week") {
        zoomDays = 7;
      } else if (gantt._tasks.unit === "month") {
        zoomDays = 30;
      }
      var width = task.duration * gantt._tasks.col_width / zoomDays;
      if (width < 20) {
  ...
  $(document).on("erui_interface_change_vertical", function (e, close, element) {
    if (close || !element) return;
    if (!document.getElementById(element).querySelector("#easy_gantt")) return;
    gantt.render();
  });

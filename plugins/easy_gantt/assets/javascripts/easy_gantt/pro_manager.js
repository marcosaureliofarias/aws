...
EasyGem.extend(ysy.proManager, {
  ...
  forLastPro: function (wrapperFunc, event) {
    var proFunctions = this.proFunctionsMap[event];
    if (!proFunctions) return;
    for (var i = proFunctions.length - 1; i >= 0; i--) {
      if (wrapperFunc.call(this, proFunctions[i])) return true;
    }
  },

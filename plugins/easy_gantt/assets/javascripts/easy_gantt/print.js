...
ysy.pro.print = {
  ...
  beforePrint: function (stripWidth) {
    ...
    if (printDateFrom && printDateTo) {
      const posDateFrom = gantt.posFromDate(moment(printDateFrom.value));
      const posDateTo = gantt.posFromDate(moment(printDateTo.value).add(1, "days"));

      if (posDateFrom && posDateTo) {
        gridWidth = -posDateFrom;
        fullWidth = posDateTo;
      }
    }
...
EasyGem.extend(ysy.pro.pdfPrint, {
  beats: 0,
  targetBeat: 2,
  patch: function () {

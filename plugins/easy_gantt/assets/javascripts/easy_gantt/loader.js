...
EasyGem.extend(ysy.data.loader, {
  /*
  ...
  load: function () {
    ...
    if (ysy.settings.sample.active) {
      ...
      if (!ysy.settings.resource.open) this.setHeading(true);
  ...
  loadSubEntity: async function (type, id) {
    if (type === "project") {
  ...
  /**
   * [setHeading change Gantt and RM tittle]
   * @param {boolean} isGantt [set true when open Gantt, set false when open RM]
   */
  setHeading: function (isGantt) {
    var settings = ysy.settings;

    if (settings.easyRedmine) {
      var $heading = $("#query_form_content").find(".easy-query-heading-wrapper");
    } else {
      $heading = $(".content-title").find("h2");
    }

    // There is nothing to change
    // For example if gantt is on a easy_page
    if (!$heading.length) {
      return
    }

    var isGlobal = settings.global;
    var labels = settings.labels.titles;
    if (isGlobal) {
      var projectName = "";
      var globalTitle = labels.global + " - ";
      var title = isGantt ? labels.easyGantt : labels.title_rm;
    } else {
      projectName = " - " + settings.project.name;
      globalTitle = "";
      title = isGantt ? labels.easyGantt : labels.title_rm_short;
    }
    var query = settings.query_name ? " - " + settings.query_name : "";
    var countHtml = "";
    if ($heading.children().length) {
      for (var i = 0; i < $heading.children().length; i++) {
        countHtml = countHtml + $heading.children()[i].outerHTML;
      }
    }
    $heading.html(ysy.main.escapeText(globalTitle + title + projectName + query) + countHtml);
    document.title = globalTitle + title + projectName;
  },
  /**
   *
  ...
  _handleMainGantt: function (data) {
    ...
    //  -- ISSUES --
    if ( (ysy.settings.resource_on && !this.buttons.onlyReservation) || !ysy.settings.resource_on ) {

...
describe("DateFromPos => PosFromDate", function () {
  ...
  it("should work for 300 random values from 0-fullWidth", function () {
    for (var i = 0; i < 300; i++) {
      ...
      expect(gantt.posFromDate(gantt.dateFromPos(value))).toBeCloseTo(value, 0); // +-0.5px
    }
  });
  it("should work for 300 random values from min_date-max_date", function () {
    for (var i = 0; i < 300; i++) {
      ...
      expect(+gantt.dateFromPos(gantt.posFromDate(value))).toBeCloseTo(+value, -5.5); // +-5 minutes
    }
  });
});

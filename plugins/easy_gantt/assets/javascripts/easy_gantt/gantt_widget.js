...
ysy.main.extender(ysy.view.Widget, ysy.view.GanttTasks, {
  _selectChildren: function () {
    ...
    combined.sort(function (a, b) {
      ...
      var aStart = a.start_date || a._start_date;
      var bStart = b.start_date || b._start_date;
...
ysy.main.extender(ysy.view.Widget, ysy.view.GanttTask, {
  _repaintCore: function () {
    ...
    if (gantt._pull[gantt_issue.id]) {
      if (!this.dhdata) {
        this.dhdata = gantt._pull[gantt_issue.id];
      }
      if (this.dhdata.parent != gantt_issue.realParent ) {
        if (!gantt._pull[gantt_issue.realParent]) {
          const tasks = ysy.view.ganttTasks.children;
          let parent;
          tasks.forEach( el => {
            if (`p${el.model.id}` === gantt_issue.realParent) {
              parent = this._constructDhData(el.model);
              parent.parent = parent.realParent;
            }
          });
          gantt.addTaskFaster(parent);
        }
  ...
  _constructDhData: function (issue) {
    ...
    if (issue.isProject) {
      ...
      if (ysy.settings.showTasksSpentTimeRatio) {
        progress = issue.spent_hours ? issue.spent_hours / issue.estimated_hours : 0;
      } else {
      ...
      $.extend(gantt_issue, {
        ...
        priority_position: issue.priority_position,
...
ysy.main.extender(ysy.view.Widget, ysy.view.GanttLink, {
  _repaintCore: function () {
    ...
    if (gantt._lpull[link.id]) {
      if (!this.dhdata) {
        this.dhdata = gantt._lpull[link.id];
      }
  ...
  _constructDhData: function (model) {
    if ( ysy.settings.unfixRelations && ysy.settings.unfixRelations.active ){
      model.delay = 0;
      model._unlocked = true;
    }
    return {
      id: model.id,
  ...
  update: function (item) {
    ...
    ysy.pro.relations.moveByCreatedLink(item, this.model);

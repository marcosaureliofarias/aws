...
ysy.data.save = function () {
  let unfixRelation = false;
  if (ysy.pro.unfixRelations && ysy.settings.unfixRelations && ysy.settings.unfixRelations.active){
    ...
    for (j = 0; j < miles.length; j++) {
      ...
      if (mile._deleted) {
        ...
        data = {
          version: {
            name: mile.name,
            status: mile.status, //the status of the version in: open (default), locked, closed
            sharing: mile.sharing, //the version sharing in: none (default), descendants, hierarchy, tree, system
            description: mile.description,
            due_date: mile.start_date.format("YYYY-MM-DD"),
            easy_version_category_id: mile.easy_version_category_id
          }
        };
        if (mile.custom_field_values) {
          data.version.custom_field_values = mile.custom_field_values;
        }

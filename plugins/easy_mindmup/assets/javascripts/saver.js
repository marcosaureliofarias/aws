(function () {
   ...
   */
  Saver.prototype.save = function (saveType) {
    var idea = this.ysy.idea;
    if (!idea || !idea.canUndo()) {
      if (this.saveType !== "off-site") {
        this.ysy.loader.load();
      }
      return;
    }
    this.saveType = saveType || "classic";
    /**@type{Array.<SendPack>} */
    var list = [];
  ...
  Saver.prototype.afterSave = function () {
    ...
    var oldSaveType = this.saveType;
    var self = this;

    this.ysy.saveInfo.saveFinished(this.saveType);
    if (fails.length > 0) {
      ...
      this.saveType = "error";
      ysy.eventBus.fireEvent("saveError");
    } else {
      ysy.util.showMessage(ysy.settings.labels.gateway.multiSuccess, "notice", 1000);
    }
    if (this.saveType !== "off-site" && this.saveType !== "error") {
      ysy.loader.load();
    } else {
        this.saveType = oldSaveType;
    }
  };
  /**
   *
  ...
  Saver.prototype.createErrorNotice = function (sendPack) {
    ...
    var reason = sendPack.response.status === 403 ? this.ysy.settings.labels.gateway.response_403 : null;
    try {
  ...
  SendPack.prototype.updateByPOST = function () {
    ...
    var layout = this.ysy.mapModel.getCurrentLayout();
    if (!source) return;
    //UPDATE ID
    this.nodeData.id = source.id;
    this.updateByPOSTAdditional(source);
    this.node.attr.isFromServer = true;
    for (let i in layout.nodes){
    if (!layout.nodes[i]) continue;
    var node = layout.nodes[i];
      if (this.node.id === node.id) {
        $("#node_" + node.id).updateNodeContent(this.node);
        this.ysy.repainter.forceRedraw(false);
      }
    }
  };
  /**
  ...
  SendPack.prototype.transformCustomValues = function (customFields, oldCustomFields) {
    ...
    if (oldCustomFields) {
      ...
      for (var j = 0; j < customIndices.length; j++) {
        if (util.startsWith(customIndices[j], "_")) continue;

(function () {
  ...
  MindMup.prototype.helperInit = function () {
    ...
    var $easyWbs = $("#easy_wbs");
    $easyWbs.find("p.nodata").remove();
    if (settings.easyRedmine && $("#content").children(".easy-content-page").length === 0) {
      $easyWbs.addClass("easy-content-page");
    }
  ...
  MindMup.prototype.setCustomData = function (idea, obj, silent) {
    ...
    if (!customFields) {
      ...
      for (var k = 0; k < props.length; k++) {
        customFields.push({"id": parseInt(props[k]), "value": obj[props[k]]});
        data._old.custom_fields[customFields.length - 1] = null;
      }
      if (!silent) {
    ...
    for (var i = 0; i < props.length; i++) {
      ...
      if (!found) {
        customFields.push({id: id, value: obj[stringId]});
        if (!data._old) data._old = {};
        if (!data._old.custom_fields) data._old.custom_fields = {};
        data._old.custom_fields[customFields.length - 1] = null;
        rev[stringId] = null;

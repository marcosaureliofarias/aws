      ...
      return this.each(function () {
        ...
        var dragStartFunc = function (event) {
          if (scrollPredicate()) {
            ...
            var target = $(event.gesture.target).closest(".mapjs-node");
            dragOrigin = {
              top: $(window).scrollTop(),
              left: element.scrollLeft(),
              withNode: target.length
            };
            if (event) {
        ...
        var dragFunc = function (e) {
          if (e.gesture && e.gesture.srcEvent) {
            ...
            if (!dragOrigin.withNode) {
              var target = $(e.gesture.target).closest(".mapjs-node");
              dragOrigin.withNode = target.length;
            }
            if (dragOrigin.withNode && !e.gesture.srcEvent.ctrlKey) {
              return dragEndFunc();
            }
        ...
        element
            .on("panstart", dragStartFunc)
            .on('panmove', dragFunc)
            .on('panend', dragEndFunc);
      });
    };

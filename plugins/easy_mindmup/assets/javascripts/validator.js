(function () {
  ...
  Validator.prototype.removeSubIdea = function (ideaId, eventOrigin, idea) {
    ...
    if (this.hasSpentTime(idea)) return false;
  ...
  Validator.prototype.hasSpentTime = function (idea) {
    let ideaHasSpentTime = false;
    this.ysy.util.traverse(idea, function (node) {
      if (node.attr.data.spent_hours) {
        ideaHasSpentTime = true;
        return;
      }
    });
    return ideaHasSpentTime;
  };

(function () {
  ...
  LegendBuilders.prototype.assigneeTemplate = '\
          <div data-item_id="{{item.id}}" class="mindmup-legend-item-cont">\
            <div class="mindmup-legend-color-box{{banned}}{{scheme}} {{#avatar}}mindmup-legend-color-box-assignee{{/avatar}}"></div>\
            {{#avatar}}\
  ...
  LegendBuilders.prototype.spentTimeTemplate = '<div data-item_id="{{spentTime}}" class="mindmup-legend-item-cont">\
            <div non-draggable="true" class="mindmup-legend-color-box{{banned}}{{scheme}}"></div>\
            {{spentTime}}\
            %\
            {{#last}}\
              +\
  ...
  LegendBuilders.prototype.spentTimeRatio = function spentTimeBuilder(item, type) {
    var lastTimeRatio = 180;
    return Mustache.render(this.spentTimeTemplate, {
      spentTime: item,
      banned: this.filter.cssByBannedValue(item),
      scheme: this.styles[type].addSchemeClass(item),
      last: item === lastTimeRatio
    });
  };

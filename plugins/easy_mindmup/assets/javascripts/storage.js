...
//###################################################################################################
  ...
  function StorageExtra(storage) {
    ...
    _getIdOfIdea = storage.ysy.getIdOfIdea;
  }
  ...
  StorageExtra.prototype.save = function (idea) {
    ...
    this.storage.savePersistentData(this._key + _getIdOfIdea(idea), JSON.stringify(toSave));
  };
  /**
   * @param {RootIdea} idea
  ...
  StorageExtra.prototype.getLocalProjectData = function (idea) {
    var json = this.storage.getPersistentData(this._key + _getIdOfIdea(idea));
  ...
  StorageExtra.prototype.enhanceData = function (data, root) {
    ...
    if (positions) {
      for (var i = 1; i < data.length; i++) {
        var nodeExtract = positions[_getIdOfIdea(data[i])];
    ...
    if (collapses) {
      for (i = 1; i < data.length; i++) {
        data[i].attr.collapsed = !!collapses[_getIdOfIdea(data[i])];
      }
    }
    return data;
  };
  ...
  StorageExtra.prototype._extractFromNode = function (node, rank, extract) {
    ...
    if (node.attr.collapsed && !_.isEmpty(node.ideas)) {
      extract.collapses[_getIdOfIdea(node)] = true;
    }
    if (node.ideas) {
    ...
    extract.positions[_getIdOfIdea(node)] = positionExtract;
    return extract;
  };

...
# Keep this section after Redmine upgrading
# Its ensure that easysoftware/Gemfile is loaded as last
# That gemfile is changning a logic of Bundler's loading

plugins_pattern = File.join(__dir__, 'plugins/*/{Gemfile,PluginGemfile}')
plugins_gemfiles = Dir.glob(plugins_pattern)

easysoftware_gemfile = File.join(__dir__, 'plugins/easysoftware/Gemfile')

plugins_gemfiles.delete(easysoftware_gemfile)
plugins_gemfiles << easysoftware_gemfile
